...
function shamanmobs:register_mob(name, def)
	minetest.register_entity(name, {
		...
		on_step = function(self, dtime)
			if self.type == "monster" and minetest.setting_getbool("only_peaceful_shamanmobs") then
				self.object:remove()
			end
...
shamanmobs.spawning_shamanmobs = {}
function shamanmobs:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, spawn_func)
	shamanmobs.spawning_shamanmobs[name] = true
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			if not shamanmobs.spawning_shamanmobs[name] then
				return
			end
			pos.y = pos.y+1
			...
			if minetest.setting_getbool("display_mob_spawn") then
				minetest.chat_send_all("[shamanmobs] Add "..name.." at "..minetest.pos_to_string(pos))
			end
...
function shamanmobs:register_arrow(name, def)
	minetest.register_entity(name, {
		physical = false,
		visual = def.visual,
		visual_size = def.visual_size,
