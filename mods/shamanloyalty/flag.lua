cf.flag_func = {
	...
	after_place_node = function(pos, placer)
		...
		if cf.players and cf.players[placer:get_player_name()] and cf.players[placer:get_player_name()].team then
			...
			if cf.teams[team].spawn and minetest.env:get_node(cf.teams[team].spawn).name == "shamanloyalty:flag" then
			...
			minetest.env:set_node(pos2,{name="shamanloyalty:flag_top_"..cf.team(team).data.color})
...
-- The flag
minetest.register_node("shamanloyalty:flag",{
	description = "Flag",
	drawtype="nodebox",
	paramtype = "light",
	walkable = false,
...
for i=1,#colors do
	...
	minetest.register_node("shamanloyalty:flag_top_"..color,{
...
minetest.register_node("shamanloyalty:flag_captured_top",{
...
minetest.register_abm({
	...
	action = function(pos)
		...
		if flag_team_data.claimed then		
			minetest.env:set_node(top,{name="shamanloyalty:flag_captured_top"})
		else
			minetest.env:set_node(top,{name="shamanloyalty:flag_top_"..cf.team(flag_team_data.team).data.color})
		end
