...
function cf.init()
	...
	local file1 = io.open(minetest.get_modpath("shamanloyalty").."/ctf.txt", "r")
	local file  = io.open(minetest.get_worldpath().."/ctf.txt", "r")
 
	if file1 and not file then
		local table = minetest.deserialize(file1:read("*all"))
		if type(table) == "table" then	
			cf.teams = table.teams
			cf.players = table.players
			cf.diplo = table.diplo
			return
		end
	end	
	if file and file1 then
